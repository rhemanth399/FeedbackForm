...
const FormEditor: React.FC = () => {
  ...
  const deleteQuestion = async (index: any) => {
    ...
    // If the question has no _id (not saved), simply remove it from state
    if (!updatedQuestions[index]._id) {
      ...
      try {
        const response = await axios.delete(`https://feedbackform-backend-ao0d.onrender.com/api/forms/${form._id}/questions/${updatedQuestions[index]._id}`);
        fetchForm(form._id!);
